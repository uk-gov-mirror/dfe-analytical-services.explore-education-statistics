  ...
  describe('full request with all parameters', () => {
    test('returns complete request with all fields', () => {
      ...
      expect(result).toEqual({
        ...
        orderBy: 'title asc',
        filter:
          "(search.in(themeId, 'theme-1', '|') or search.in(publicationId, 'pub-3', '|')) and search.in(releaseType, 'AccreditedOfficialStatistics', '|') and geographicLevelDetails/any(g: search.in(g/code, 'NAT', '|')) and latestData eq true and api/id ne null and api/id ne ''",
      });
    });
  });
});
